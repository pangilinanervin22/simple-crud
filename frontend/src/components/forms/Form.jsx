	...
	renderButtonSubmit(label, handleClick) {
		return (
			<button
				...
				className={`${style.btn_submit} ${this.buttonDesign(this.validate())}`}
				onClick={handleClick}
			>
				{label}
			</button>
...
// input component
const FormInput = ({ value, handleChange, label, name, type, error, defaultValue }) => {
	return (
...
//Dropdown component
const FormDropdown = ({ value, options, onChange, label, name, error, type }) => {
	return (
		<div className={style.form_group}>
			<label htmlFor={name} className={style.label}>
				{label}
